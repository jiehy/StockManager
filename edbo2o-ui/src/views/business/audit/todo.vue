<template>
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="text"
            @click="handlePackageReview(scope.row)"
            v-hasPermi="['flowdefine:audit:packagereview']"
          >审核
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="审核" :visible.sync="reviewOpen" width="500px" append-to-body>
      <el-form ref="form" :model="review" :rules="rules" label-width="80px">
        <el-form-item label="审核" prop="serviceItemId">
          <el-select v-model="review.audit" placeholder="请选择审核结果" clearable>
            <el-option
        ...
        <el-form-item label="意见" prop="serviceItemName">
          <el-input v-model="review.opinion" placeholder="请输入意见"/>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="reviewForm">确 定</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        choose: [{label: '同意',value: 1},{label: '拒绝',value: 0}],
        reviewOpen: false,
        historyList: [],
        historyOpen: false,
        progressOpen: false,
    ...
    methods: {
      ...
      reviewForm() {
        let params = {
          id: this.review.id,
          audit: this.review.audit,
          opinion: this.review.opinion
        };
        packageReviewAudit(params).then(()=>{
          this.review = {
            id: undefined,
            audit: undefined,
            opinion: undefined
      ...
      getList() {
        this.loading = true;
        todoAudit(this.queryParams).then(response => {
