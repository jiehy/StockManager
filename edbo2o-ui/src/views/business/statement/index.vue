      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['appointment:statementItem:detail']"
          >明细
          </el-button>
          <el-button
            size="mini"
          ...
          <el-button
            ...
            v-hasPermi="['appointment:statement:sync']"
          >同步信息
          </el-button>
        </template>
      </el-table-column>
...
<script>
  import {
    ...
    updateStatement,
    syncStatement
  } from "@/api/business/statement/statement";

  export default {
    name: "Statement",
    dicts: ['service_type','statement_status'],
    data() {
      return {
        // 遮罩层
        loading: true,
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listStatement(this.addDateRange(this.queryParams,this.dateRange)).then(response => {
      ...
      /** 同步信息按钮操作 */
      handleSync(row) {
        syncStatement(row).then(response => {
